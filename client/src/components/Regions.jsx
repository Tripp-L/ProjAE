...
const initialCivilizations = [
  ...
  },
  {
    id: 30,
      ...
      imageurl: "https://cdn.britannica.com/22/60122-050-61780978/singing-corroboree-Aborigines-event-Australian-Dreaming-activities.jpg",
      dates: "65,000 BC – Present",
      regions: "Australia",
      latitude: -25.2744, 
      longitude: 133.7751,
      videoUrl: "xQHhDtGTaAk",
      leaders: [],
      deities: [
...
function extractYouTubeVideoID(url) {
  ...
  return matches ? matches[1] : url; // If it's not a URL, assume it's already a video ID
}
...
function Regions() {
  ...
  const markers = [...initialCivilizations, ...initialEvents, ...initialArtifacts].map((item, index) => ({
    id: index,
    name: item.name || item.title,
    coordinates: [item.latitude, item.longitude],
    description: item.description || item.cardDetailedText,
    imageUrl: item.imageUrl || item.imageurl || (item.media && item.media.source.url),
    videoUrl: item.videoUrl,
    dates: item.dates || item.date || item.discovery_date,
    books: item.books || [],
  }));
  ...
  return (
    <div style={{ display: 'flex', height: '100vh' }}>
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        {selectedItem && (
          ...
          >
            ...
            <img src={selectedItem.imageUrl} alt={selectedItem.name} className="info-image" style={{ width: '100%', height: 'auto' }} />
            <p><strong>Dates:</strong> {selectedItem.dates}</p>
            <p>{selectedItem.description}</p>
            {selectedItem.videoUrl && (
              <YouTube
                videoId={extractYouTubeVideoID(selectedItem.videoUrl)}
                opts={{
                  height: '200',
                  width: '100%',
                  playerVars: {
      ...
      <div style={{ width: '300px', padding: '10px', overflowY: 'scroll', backgroundColor: '#f9f9f9', borderLeft: '1px solid #ddd' }}>
        <h3>Book Recommendations</h3>
        {selectedItem && selectedItem.books && selectedItem.books.length > 0 ? (
          selectedItem.books.map((book, index) => (
            <div key={index} className="book-card" style={{ marginBottom: '20px', padding: '10px', backgroundColor: '#fff', borderRadius: '5px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
              <h4>{book.title}</h4>
